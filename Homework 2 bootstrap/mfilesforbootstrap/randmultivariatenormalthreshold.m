...
function ans = randmultivariatenormalthreshold(m,Sigma,thr,label,maxit)

% randmultivariatenormalthreshold -- generates (pseudo) random vectors 
%                         Y = (X|label = (abs(X) > thr))
%  Usage
%    ans = randmultivariatenormalthreshold(m,Sigma,thr,label);
%    ans = randmultivariatenormalthreshold(m,n,thr,label);
%  Inputs
...
%    label  binary vector; default is ones(n,1)
...
if n>1 & length(label) == 1,
   label = label*ones(n,1);
end
if nargin < 5,
   maxit = 10;
end
...
if label(1) == 1,
   ...
   X(1,1:m) = stdev1*randninsideinterval(1,m,-thr/stdev1,thr/stdev1);
end
for i = 2:n,
   ...
   if label(i) == 1,
      ...
      X(i,1:m) = mui+stdevi*...
                 randninsideinterval(1,m,(-thr-mui)/stdevi,(thr-mui)/stdevi);
   end
end
